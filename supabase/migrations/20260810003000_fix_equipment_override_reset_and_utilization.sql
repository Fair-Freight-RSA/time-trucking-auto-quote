alter table public.vehicle_recommendations
  add column if not exists system_number_of_trucks integer,
  add column if not exists system_payload_utilization_percent numeric(7, 2),
  add column if not exists system_volume_utilization_percent numeric(7, 2),
  add column if not exists system_deck_utilization_percent numeric(7, 2),
  add column if not exists equipment_override_history jsonb not null default '[]'::jsonb;
...
update public.vehicle_recommendations
 ...
 where system_number_of_trucks is null
    or system_payload_utilization_percent is null
    or system_volume_utilization_percent is null
    or system_deck_utilization_percent is null;
...
begin
  ...
  select coalesce(sum(coalesce(
    ...
    into total_weight, total_volume, total_deck_area
  ...
  if target_equipment_profile_id is null then
    ...
    target_units := greatest(1, coalesce(recommendation_record.system_number_of_trucks, recommendation_record.number_of_trucks, 1));
